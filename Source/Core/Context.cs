...
using static NativeBindings.Context;

/// <summary>
/// Wraps SPIRV-Cross context functionality provided by <see cref="NativeBindings.Context"/> into a type-safe and memory-safe object.
...
{
    internal NativeBindings.Context* context;
    ...
    public Context()
    {
        ...
        fixed (NativeBindings.Context** contextRef = &context)
        {
    ...
    public ParsedIR ParseSpirv(byte[] spirvWords)
    {
        ...
        NativeBindings.ParsedIR* parsedIR = null;
    ...
    private NativeBindings.Compiler* CreateCompiler(ParsedIR parsedIR, NativeBindings.Backend backend)
    {
        NativeBindings.Compiler* compiler = null;

        Throw(spvc_context_create_compiler(context, backend, parsedIR.nativeIR, NativeBindings.CaptureMode.Copy, &compiler));

        return compiler;
    }

    private NativeBindings.Compiler* CreateCompiler(byte[] spirvWords, NativeBindings.Backend backend)
    {
        NativeBindings.Compiler* compiler = null;
        
        ParsedIR ir = ParseSpirv(spirvWords);
        
        Throw(spvc_context_create_compiler(context, backend, ir.nativeIR, NativeBindings.CaptureMode.TakeOwnership, &compiler));

        return compiler;
    }

    internal unsafe void Throw(NativeBindings.Result result)
    {
        switch (result)
        {
            case NativeBindings.Result.InvalidArgument:  throw new InvalidArgumentException(Marshal.PtrToStringUTF8((IntPtr)spvc_context_get_last_error_string(context)) ?? "");
            case NativeBindings.Result.InvalidSPIRV:     throw new InvalidSPIRVException(Marshal.PtrToStringUTF8((IntPtr)spvc_context_get_last_error_string(context)) ?? "");
            case NativeBindings.Result.UnsupportedSPIRV: throw new UnsupportedSPIRVException(Marshal.PtrToStringUTF8((IntPtr)spvc_context_get_last_error_string(context)) ?? "");
            case NativeBindings.Result.OutOfMemory:      throw new OutOfMemoryException(); 
        };
    }
...
// -----------
    ...
    public Reflector CreateReflector(ParsedIR parsedIR)
        => new(this, CreateCompiler(parsedIR, NativeBindings.Backend.None));
    ...
    public Json.JsonReflector CreateJsonReflector(ParsedIR parsedIR)
        => new(this, CreateCompiler(parsedIR, NativeBindings.Backend.Json));
    ...
    public HLSL.HLSLCrossCompiler CreateHLSLCompiler(ParsedIR parsedIR)
        => new(this, CreateCompiler(parsedIR, NativeBindings.Backend.HLSL));
    ...
    public MSL.MSLCrossCompiler CreateMSLCompiler(ParsedIR parsedIR)
        => new(this, CreateCompiler(parsedIR, NativeBindings.Backend.MSL));
    ...
    public GLSL.GLSLCrossCompiler CreateGLSLCompiler(ParsedIR parsedIR)
        => new(this, CreateCompiler(parsedIR, NativeBindings.Backend.GLSL));
    ...
    public CPP.CPPCrossCompiler CreateCPPCompiler(ParsedIR parsedIR)
        => new(this, CreateCompiler(parsedIR, NativeBindings.Backend.CPP));
...
// -----------
    ...
    public Reflector CreateReflector(byte[] spirvWords)
        => new(this, CreateCompiler(spirvWords, NativeBindings.Backend.None));
    ...
    public Json.JsonReflector CreateJsonReflector(byte[] spirvWords)
        => new(this, CreateCompiler(spirvWords, NativeBindings.Backend.Json));
    ...
    public HLSL.HLSLCrossCompiler CreateHLSLCompiler(byte[] spirvWords)
        => new(this, CreateCompiler(spirvWords, NativeBindings.Backend.HLSL));
    ...
    public MSL.MSLCrossCompiler CreateMSLCompiler(byte[] spirvWords)
        => new(this, CreateCompiler(spirvWords, NativeBindings.Backend.MSL));
    ...
    public GLSL.GLSLCrossCompiler CreateGLSLCompiler(byte[] spirvWords)
        => new(this, CreateCompiler(spirvWords, NativeBindings.Backend.GLSL));
    ...
    public CPP.CPPCrossCompiler CreateCPPCompiler(byte[] spirvWords)
        => new CPP.CPPCrossCompiler(this, CreateCompiler(spirvWords, NativeBindings.Backend.CPP));

    /// <summary>
    /// Disposes of the NativeBindings SPIRV-Cross context and all the resources it owns.
